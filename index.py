...
def write_to_sheet(table, earnings_date, starting_row_xl_index):
    ...
    for i in range(1, len(earnings_rows)):
        ...
        earnings_date_reformatted = '{}/{}/{}'.format(earnings_date.strftime('%m'), earnings_date.strftime('%d'), earnings_date.strftime('%Y'))
        sheet1.write(starting_row_xl_index + i, 0, earnings_date_reformatted)
...
def create_weekdates_list():
    ...
    number_of_weeks = round((datetime.date.today() + datetime.timedelta(365) - first_monday_date).days / 7)
    for i in range(number_of_weeks):
        week = []
        for j in range(5):
            day_date = first_monday_date + datetime.timedelta((7 * i) + j)
            week.append(day_date)
        weekdates_of_years.append(week)
    return weekdates_of_years   
...
for week in weekdates_list:
    ...
    for i in range(5):
        ...
        try:
            ...
            break
        starting_row_xl_index += (len(table.find_all('tr')) - 1)

save_workbook('./yahoo-finance-earnings.xls')